...
# value of the average weight at the endpoints of each piece.
# Hence, there will be an even number of rows. There will be 3 columns:
#   1. value of u
...
function compute_average_weights(tp::TargetParameter)
    if tp.name == "LATE(u₁, u₂)"
        u₁ = tp.int_limits(1)[1] # only coded 1-dim case
        u₂ = tp.int_limits(1)[2] # only coded 1-dim case
